#!/usr/bin/env python3
"""
Arabic EOU Synthetic Data Generator
...
Author: Arabic EOU Project Team
Date: December 2025
"""
...
class ArabicEOUDataGenerator:
            ...
            for idx, turn in enumerate(data['dialogue'], 1):
                if isinstance(turn, dict) and 'speaker' in turn and 'text' in turn:
                    normalized_turn = {
                        ...
                        'has_continuation_signals': turn.get('has_continuation_signals', False)
                    }
    ...
    def generate_conversation(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if normalized:
                    logger.debug(f"Successfully generated conversation: {normalized.get('conversation_id')}")
                    return normalized
                else:
                    logger.warning(f"Attempt {attempt + 1}/{max_retries}: Failed to normalize conversation format")
    ...
    def generate_batch(
        ...
        logger.info(f"Starting batch generation: {len(prompts)} conversations")
        ...
        for idx, prompt in enumerate(prompts, 1):
            ...
            if conversation:
                ...
                logger.info(f"Success: Generated conversation {idx}")
            else:
                stats['failed_generations'] += 1
                logger.warning(f"Failed: Could not generate conversation {idx}")
        ...
        logger.info(f"Batch complete: {stats['successful_generations']}/{len(prompts)} successful")
        return stats
    ...
    def validate_conversations(self, conversations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(conversations) > 0:
            report['statistics']['avg_turns_per_conversation'] = total_turns / len(conversations)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate synthetic Arabic conversational data for EOU detection'
    )
    parser.add_argument(
        '--num-conversations',
        type=int,
    ...
    parser.add_argument(
        '--output-file',
        type=str,
        default='arabic_eou_conversations.json',
        help='Output file path (default: arabic_eou_conversations.json)'
    )
    parser.add_argument(
        '--log-file',
        type=str,
        default='arabic_eou_generation.log',
        help='Log file path (default: arabic_eou_generation.log)'
    )
    parser.add_argument(
        '--temperature',
        type=float,
    ...
    parser.add_argument(
        ...
        help='Model to use (default: Qwen/Qwen3-235B-A22B-Instruct-2507)'
    )
    parser.add_argument(
        '--validate',
        action='store_true',
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Arabic EOU Data Generator")
        logger.info("=" * 80)
        
        generator = ArabicEOUDataGenerator(model_name=args.model)
        
        # Use conversation prompts
        prompts = CONVERSATION_PROMPTS[:args.num_conversations]
        ...
        logger.info(f"  - Conversations: {len(prompts)}")
        ...
        if args.validate:
            ...
            logger.info(f"Total conversations: {validation_report['total_conversations']}")
            logger.info(f"Valid conversations: {validation_report['valid_conversations']}")
            logger.info(f"Invalid conversations: {validation_report['invalid_conversations']}")
            logger.info(f"Average turns per conversation: {validation_report['statistics']['avg_turns_per_conversation']:.2f}")
            ...
            logger.info(f"EOU ratio: {validation_report['statistics']['avg_eou_ratio']:.2%}")
            
            if validation_report['issues']:
                logger.warning(f"Found {len(validation_report['issues'])} issues:")
