...
def split_dataset(
    input_file, output_dir, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, seed=42
):
    ...
    eou_train, eou_val, eou_test = calculate_splits(
        eou_samples, train_ratio, val_ratio, test_ratio
    )
    non_eou_train, non_eou_val, non_eou_test = calculate_splits(
        non_eou_samples, train_ratio, val_ratio, test_ratio
    )
...
def write_report(output_dir, train_stats, val_stats, test_stats):
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        if (
            abs(train_eou_ratio - val_eou_ratio) < 0.05
            and abs(train_eou_ratio - test_eou_ratio) < 0.05
        ):
        ...
        if (
            train_stats["total"] > val_stats["total"]
            and train_stats["total"] > test_stats["total"]
        ):
