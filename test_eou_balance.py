...
def test_eou_distribution(num_conversations: int = 5):
    ...
    prompts = prompt_builder.get_all_eou_aware_prompts(
        style="asr_like", num_turns=10, target_non_eou_ratio=0.4
    )
    ...
    for i in range(num_conversations):
        ...
        try:
            ...
            print(
                f"  EOU=true: {eou_true} ({eou_true/len(conversation.turns)*100:.1f}%)"
            )
            print(
                f"  EOU=false: {eou_false} ({eou_false/len(conversation.turns)*100:.1f}%)"
            )
